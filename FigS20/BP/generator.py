...
def generateclause(n, m):
    # Generates m random clauses with 3 distinct variables from range [1, n].
...
def printclause(clausein, m):
    # Prints the list of clauses in a formatted, concise layout.
    clause = clausein.copy()
    for i in range(m):
        # Concise print: C{index}:v1,v2,v3
        print(f"C{i}:{clause[i][0]},{clause[i][1]},{clause[i][2]}", end="  ")
        if(i % 7 == 6):
            print("") 
    print("") 

def build_matrix_from_clauses(clauses):
    # Constructs the binary coefficient matrix A and vector b from the clause list.
    if len(clauses) == 0:
...
def rref_gf2(A_in, b_in):
    # Computes the Row Reduced Echelon Form (RREF) of the system over GF(2).
    A = A_in.copy() % 2
    b = b_in.copy() % 2
    m, n = A.shape
    row = 0
...
def solve_linear_mod2(A, b):
    # Solves the linear system Ax = b (mod 2) and returns the nullspace basis.
    m, n = A.shape
...
def verify(m, clausesin, p2in):
    # Verifies if the computed solution satisfies all clauses modulo 2.
...
def generaterandom(n, m):   
    # Orchestrates clause generation, index normalization, solving, and returning results.
