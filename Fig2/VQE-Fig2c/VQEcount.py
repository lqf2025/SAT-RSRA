...
def mostcommonstate(n, p2in, parameter, dim):
    """Infer the most likely bitstring by thresholding parameters and applying the induced flips."""
    p2 = p2in.copy()
    r = parameter.copy()
    r = r / np.pi
    r = np.rint(r) % 2
...
def VQEnAdam(clauses, initialparametersin, n, m, lr, maxepoch, p2in, dim, b1, b2, eposilon):
    """Run a Nadam-style optimizer on the VQE parameters and return the final energy and state."""
    state = 0
...
def verify(clauses, m, s):
    """Check whether bitstring s satisfies all one-in-three constraints in the first m clauses."""
    e = 0
    for i in range(m):
        l1 = clauses[i][0]
        l2 = clauses[i][1]
...
def readnpz(n, m):
    """Load clauses and p2 from a dataset file named by (n,m)."""
