...
def slope(plist,run):
    ...
    for n in range(20,40):
        ...
        for pos in range(plen): 
            ...
            @parfor(range(2000), disable=True)
            def single2(i):
                return QAAposs(single[i][0],single[i][1],plist[pos]),QAApossqs(single[i][2],single[i][3],plist[pos])
            single2=np.array(single2)
            sloperecord[n-20][pos]=-np.log(sum(single2[:,0])/2000)
    scale=np.zeros((plen))
    for pos in range(plen):
        slope, intercept = np.polyfit(range(20,40),sloperecord[:,pos], 1)
        scale[pos]=np.e**slope
        print(plist[pos],np.e**slope)
    np.savez('QAAscale'+' '+str(run)+'.npz',scale=scale,plist=plist)
