import numpy as np

def mean_boot(x, ci=0.95, B=2000):
    """Return a percentile-bootstrap confidence interval for the sample mean."""
    x = np.asarray(x, dtype=float)
    n = x.shape[0]
    a = 1.0 - ci

    rng = np.random.default_rng()  # Use a non-fixed seed.
    idx = rng.integers(0, n, size=(B, n))
    boot = x[idx].mean(axis=1)

    lo = np.quantile(boot, a / 2.0)
    hi = np.quantile(boot, 1.0 - a / 2.0)
    return float(lo), float(hi)


def fit_boot(n_list, Y, ci=0.95, B=2000, eps=1e-12):
    """Bootstrap mean curves and fit mean(n) \approx a b^n in log-space, returning percentile CIs for (a,b)."""
    rng = np.random.default_rng()  # Use a non-fixed seed.

    n = np.asarray(n_list, dtype=float)
    Y = np.asarray(Y, dtype=float)
    N, T = Y.shape
    alpha = 1.0 - ci

    X = np.column_stack([np.ones(N, dtype=float), n])
    pinv = np.linalg.inv(X.T @ X) @ X.T

    idx = rng.integers(0, T, size=(B, N, T))
    means = Y[np.arange(N)[None, :, None], idx].mean(axis=2)

    logy = np.log(np.clip(means, eps, None))
    beta = (pinv @ logy.T).T

    a_boot = np.exp(beta[:, 0])
    b_boot = np.exp(beta[:, 1])

    qlo, qhi = alpha / 2, 1 - alpha / 2
    a_ci = (float(np.quantile(a_boot, qlo)), float(np.quantile(a_boot, qhi)))
    b_ci = (float(np.quantile(b_boot, qlo)), float(np.quantile(b_boot, qhi)))
    return a_ci, b_ci
