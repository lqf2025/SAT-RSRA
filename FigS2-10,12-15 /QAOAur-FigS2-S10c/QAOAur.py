...
# ============================================================
# Unrestricted (UR) Hamiltonian Construction
# ============================================================
def build_diag_and_mask_ur_fast(dim: int, clauses_np: np.ndarray):
    # Computes the diagonal Hamiltonian and solution mask for Unrestricted 3-SAT instances.
    
    clauses = np.asarray(clauses_np, dtype=np.int32)
    if clauses.ndim != 2 or clauses.shape[1] < 3:
        raise ValueError("UR clauses must be 2D with >=3 columns (1-indexed vars).")

    K = 1 << dim
    S = _get_S(dim)  # int8[dim, K]
    diag = np.zeros((K,), dtype=np.int32)

    # Vectorized accumulation of penalties
    for row in clauses:
        l1, l2, l3 = int(row[0]), int(row[1]), int(row[2])
        x1 = S[l1 - 1]
        x2 = S[l2 - 1]
        x3 = S[l3 - 1]
        tmp = (x1 + x2 + x3 - 1).astype(np.int32, copy=False)
        diag += tmp * tmp

    mask = (diag == 0).astype(np.float32, copy=False)
...
# Problem Generation
# ============================================================
def generatecase_with_diags_ur(n: int, k: float, seed: int):
    # Generates a random solvable Unrestricted 3-SAT instance and its Hamiltonian diagonal.
    random.seed(seed)
    ...
    while True:
        clauses, _p2 = generator.generaterandom(n, m)  # UR does not need p2
        dim, diag, mask = build_diag_and_mask_ur_fast(n, clauses)
        if mask.max() > 0.5:
            return dim, diag, mask
...
def eval_full_test_parallel(testset, guess, ctx, nproc=8, chunk_size=64):
    ...
    chunks = make_chunks(idx, chunk_size)
...
def data_ur(layer, k, tryn,
    ...
    for n in range(5, 12):
        print("\n==============================")
        print(f"UR  n = {n}, k = {k}, try = {tryn}", flush=True)

        # ---------- Generate Train/Test Data ----------
        trainset = []
        for i in range(train):
            dim, diag, mask = generatecase_with_diags_ur(
                n, k, seed=10_000_000 + 131*n + 1009*tryn + i
            )
        ...
        for i in range(test):
            dim, diag, mask = generatecase_with_diags_ur(
                n, k, seed=20_000_000 + 131*n + 1009*tryn + i
            )
        ...
        print(f"  test_success = {test_success:.8f}", flush=True)
        print(f"  -log(test_success) = {success[n-5]:.8f}", flush=True)

    out_name = f"QAOAur{k}try{tryn}.npz"
