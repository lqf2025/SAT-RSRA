...
def _default_out_path(npz_path):
    """
    Generate the output path by changing the output directory to 'QAArecoverwhole' 
    ...
    stem = stem.replace("QAAsingles", "recoverQAA", 1)

    # Set the output directory to 'QAArecoverwhole'
    out_dir = "QAArecoverwhole"
...
def recover_QAAsinglec(
    ...
    if out_path is None:
        out_path = _default_out_path(npz_path)  # Output to 'QAArecoverwhole' folder

    # Save the results to the 'QAArecoverwhole' folder
    np.savez_compressed(
        out_path,
        src=npz_path,
        n_list=n_list,
...
if __name__ == "__main__":
    ...
    for k in [0.2,0.4, 0.8, 0.9]:
        # Process each file and save the results to the 'QAArecoverwhole' folder
